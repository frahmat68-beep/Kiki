<x-guest-layout>
    <div class="space-y-5">
        <div>
            <h1 class="text-xl font-semibold text-slate-900">Verifikasi OTP</h1>
            <p class="mt-2 text-sm text-slate-600">Masukkan 6 digit kode OTP yang dikirim ke email kamu.</p>
        </div>
        ...
        <form method="POST" action="{{ route('otp.verify') }}" class="space-y-4">
            @csrf
            <div>
                <label for="otp" class="text-xs font-semibold uppercase tracking-widest text-slate-500">Kode OTP</label>
                <input
                    id="otp"
                    name="otp"
                    type="text"
