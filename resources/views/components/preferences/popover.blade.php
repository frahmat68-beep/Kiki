...
<div {{ $attributes->class(['manake-preferences-popover rounded-[1.6rem] border p-4 shadow-2xl']) }}>
    <div class="space-y-4">
        <section class="space-y-2.5">
            ...
            <div class="grid grid-cols-2 gap-2">
                <a
                    ...
                    class="rounded-2xl border px-3 py-2 text-center text-sm font-semibold {{ $locale === 'id' ? '!text-slate-900' : '!text-slate-700' }}"
                >
                    {{ __('ui.languages.id') }}
                </a>
                <a
                    ...
                    class="rounded-2xl border px-3 py-2 text-center text-sm font-semibold {{ $locale === 'en' ? '!text-slate-900' : '!text-slate-700' }}"
                >
                    {{ __('ui.languages.en') }}
                </a>
            </div>
            ...
            <div class="grid grid-cols-1 gap-2">
                @foreach (['system' => __('ui.settings.theme_system'), 'dark' => __('ui.settings.theme_dark'), 'light' => __('ui.settings.theme_light')] as $value => $label)
                    <a
                        ...
                        class="rounded-2xl border px-3 py-2 text-left text-sm font-semibold {{ $currentTheme === $value ? '!text-slate-900' : '!text-slate-700' }}"
                    >
                        {{ $label }}
                    </a>
                @endforeach
