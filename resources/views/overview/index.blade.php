            ...
            <div class="mt-5 space-y-4">
                @forelse ($activeRentals as $order)
                    ...
                    <article class="rounded-xl border border-slate-100 p-4">
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm font-semibold text-slate-900">{{ $order->order_number ?? ('ORD-' . $order->id) }}</p>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                @if (($order->status_pembayaran ?? 'pending') === 'paid' && ! $order->hasOutstandingDamageFee())
                                    <a href="{{ route('account.orders.receipt', $order) }}" class="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:text-blue-600 hover:border-blue-200">
                                        Resi
                                    </a>
                                ...
                                @endif
                                <a href="{{ route('account.orders.show', $order) }}" class="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:text-blue-600 hover:border-blue-200">
                                    Detail
                                </a>
                            </div>
                        </div>
